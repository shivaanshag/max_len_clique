...
////        pAlgorithm->AddCallBack(verifyIndependentSetMatrix);
    ...
    list<int> max_len_cliques;
    auto storeMaxClique = [&max_len_cliques](list<int> const &clique) {
        if (max_len_cliques.size() < clique.size()){
            max_len_cliques.clear();
            max_len_cliques = clique;
        }
    };
    pAlgorithm->AddCallBack(storeMaxClique);
    RunAndPrintStats(pAlgorithm, cliques, bTableMode);

    for (int ele: max_len_cliques){
        std::cout << ele << " ";
    }
    std::cout << std::endl;
